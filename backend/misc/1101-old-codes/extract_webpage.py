...
def extract_content_html(url):
    ...
    try:
        ...
        # Add webpage title and source URL at the top
        title = soup.find('title')
        if title:
            html_content.append(f"<h1>{title.string}</h1>")
        html_content.append(f'<p><em>Source: <a href="{url}">{url}</a></em></p>')
        ...
        if 'wikipedia.org' in url:
            ...
            if content_div:
                html_content.extend(process_content(content_div, url))
        
        # Generic webpage handling
        else:
            main_content = extract_main_content(soup)
            if main_content:
                html_content.extend(process_content(main_content, url))
        
        # Add MathJax script for LaTeX rendering and basic HTML structure
        html_template = f"""
        <!DOCTYPE html>
        <html>
        <head>
...
def process_content(container, base_url):
    """
    ...
    for element in container.find_all(['p', 'h2', 'h3', 'h4', 'ul', 'ol', 'img']):
        content.extend(process_element(element, base_url))
    
    return content

def process_element(element, base_url):
    """
...
def save_content_html(url, target_path):
    """
...
if __name__ == "__main__":
    ...
    target_path = "source_html/moment_of_inertia.html"
    saved_file = save_content_html(url, target_path)
    if saved_file:
