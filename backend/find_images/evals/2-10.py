...
def search_images(query, api_key, cx, num_images=10):  # Changed default to 10
    """
    ...
    params = {
        ...
        'cx': cx,
        'searchType': 'image',
        'num': num_images  # Now fetching 10 images
    }
...
def download_image(url, folder_path, index):
    ...
    try:
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
            ...
            os.remove(file_path)  # Remove empty or corrupt files
            return None
...
def process_search_results(suggestion, api_key, cx, base_dir="image_searches"):
    ...
    # Download images and track successful downloads
    successful_downloads = []
    for i, result in enumerate(results):
        filename = download_image(result['link'], folder_path, len(successful_downloads))
        if filename:
            result_with_filename = result.copy()
            result_with_filename['saved_filename'] = filename
            successful_downloads.append(result_with_filename)
            if len(successful_downloads) >= 5:  # Stop after getting 5 successful downloads
                break
        time.sleep(0.5)  # Add delay between downloads
    
    # Save metadata only for successfully downloaded images
    metadata = {
        'query': query,
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'original_quote': suggestion['quote'],
        'section_title': suggestion['section_title'],  # Added section_title to metadata
        'reason': suggestion['reason'],
        'description': suggestion['image_description'],
        'images': successful_downloads[:5]  # Only keep the first 5 successful downloads
    }
